...
REFERRAL_COMMISSIONS = {
    "A1": [35,17,12,8,5,5], "A2": [60,25,17,12,5,5], "A3": [90,35,21,14,5,5],
    "B1": [140,85,35,20,9,9], "B2": [220,125,90,30,15,15], "B3": [350,175,100,32,18,18],
}
PACKAGES = {
...
ST_PHONE="W_PHONE"; ST_RECEIPT="W_RECEIPT"; ST_WALLET="W_WALLET"
ST_WITHDRAW="W_WITHDRAW"; ST_BROADCAST="W_BROADCAST"
ST_SETWALLET="W_SETWALLET"; ST_ADDLINK="W_ADDLINK"; ST_SCREENSHOT="W_SCREENSHOT"

BTN_TASKS="📋 مهامي اليومية"; BTN_WALLET="💰 محفظتي"; BTN_PACKAGES="📦 الباقات"
BTN_REFERRAL="👥 إحالاتي"; BTN_WITHDRAW="💸 سحب الأرباح"; BTN_HELP="ℹ️ مساعدة"
BTN_BACK="🔙 رجوع"; BTN_HOME="🏠 الرئيسية"; BTN_ADMIN="👑 لوحة الأدمن"

logging.basicConfig(level=logging.INFO)

def main_kb():
    return ReplyKeyboardMarkup([[BTN_TASKS,BTN_WALLET],[BTN_PACKAGES,BTN_REFERRAL],[BTN_WITHDRAW,BTN_HELP]],resize_keyboard=True,persistent=True)
def back_kb():
...
def init_db():
    conn=sqlite3.connect("teamo.db"); c=conn.cursor()
    c.execute("""CREATE TABLE IF NOT EXISTS users(
        user_id INTEGER PRIMARY KEY,serial_no INTEGER,username TEXT,full_name TEXT,
        phone TEXT,balance REAL DEFAULT 0,package TEXT DEFAULT NULL,wallet TEXT DEFAULT NULL,
        ref_code TEXT UNIQUE,referred_by INTEGER DEFAULT NULL,joined_at TEXT,
        package_expires TEXT DEFAULT NULL,is_admin INTEGER DEFAULT 0)""")
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS packages_history(
        id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER,package TEXT,price REAL,
        status TEXT DEFAULT 'pending',receipt TEXT,created_at TEXT)""")
    c.execute("""CREATE TABLE IF NOT EXISTS daily_tasks(
        id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER,task_date TEXT,
        tasks_done INTEGER DEFAULT 0,earned REAL DEFAULT 0)""")
    c.execute("""CREATE TABLE IF NOT EXISTS task_screenshots(
        id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER,task_date TEXT,task_num INTEGER,
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS withdrawals(
        id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER,amount REAL,wallet TEXT,
        status TEXT DEFAULT 'pending',created_at TEXT)""")
    c.execute("""CREATE TABLE IF NOT EXISTS referral_earnings(
        id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER,from_user INTEGER,level INTEGER,
    ...
    for col in ["phone TEXT","package_expires TEXT","is_admin INTEGER DEFAULT 0"]:
...
def get_setting(k):
    conn=db();c=conn.cursor();c.execute("SELECT value FROM settings WHERE key=?",(k,));r=c.fetchone();conn.close();return r[0] if r else None
def set_setting(k,v):
    conn=db();c=conn.cursor();c.execute("INSERT OR REPLACE INTO settings(key,value)VALUES(?,?)",(k,v));conn.commit();conn.close()
...
def get_user(uid):
    conn=db();c=conn.cursor();c.execute("SELECT * FROM users WHERE user_id=?",(uid,));u=c.fetchone();conn.close();return u

def is_admin(uid):
    if uid==MAIN_ADMIN: return True
    u=get_user(uid); return bool(u and len(u)>12 and u[12])

def gen_ref(uid): return f"TMO{uid:06d}"

def register_user(uid,username,full_name,phone,referred_by=None):
    conn=db();c=conn.cursor()
    c.execute("SELECT user_id FROM users WHERE user_id=?",(uid,))
    if c.fetchone(): conn.close(); return False
    serial=next_serial()
    c.execute("INSERT INTO users(user_id,serial_no,username,full_name,phone,ref_code,referred_by,joined_at)VALUES(?,?,?,?,?,?,?,?)",
        (uid,serial,username,full_name,phone,gen_ref(uid),referred_by,datetime.now().isoformat()))
    conn.commit();conn.close();return True

def get_user_by_ref(ref):
    conn=db();c=conn.cursor();c.execute("SELECT user_id FROM users WHERE ref_code=?",(ref,));r=c.fetchone();conn.close();return r[0] if r else None

def get_ancestors(uid,levels=6):
    anc=[];conn=db();c=conn.cursor();cur=uid
    for _ in range(levels):
        c.execute("SELECT referred_by FROM users WHERE user_id=?",(cur,));row=c.fetchone()
        if not row or not row[0]: break
        anc.append(row[0]);cur=row[0]
    conn.close();return anc

def distribute_commissions(new_uid,pkg):
    comms=REFERRAL_COMMISSIONS.get(pkg,[]);ancs=get_ancestors(new_uid)
    conn=db();c=conn.cursor()
...
def pkg_active(user):
    ...
    try: return datetime.now()<=datetime.strptime(user[11],"%Y-%m-%d")
    except: return True
...
def get_task_link():
    ...
    r=c.fetchone();conn.close();return r[0] if r else "https://youtube.com"

def get_all_admins():
    admins=[MAIN_ADMIN];conn=db();c=conn.cursor()
...
async def send_home(message,uid):
    ...
    earned=today_row[4] if today_row else 0.0
    name=u[3] or u[2] or "—"
    exp=f"\n📅 تنتهي: *{u[11]}*" if u[11] else ""
...
async def start(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        ...
        if ruid and ruid!=uid: referred_by=ruid; referrer=get_user(ruid)
    existing=get_user(uid)
    if existing:
        await send_home(update.message,uid); return
    # في منتصف التسجيل
    if context.user_data.get("state")==ST_PHONE:
        await update.message.reply_text("📱 أرسل رقم هاتفك:\n\n🇮🇶 `+9647701234567`",parse_mode="Markdown"); return
    ...
    await update.message.reply_text(
        f"👋 *أهلاً وسهلاً بك في TEAMO!*\n\n🚀 منصة المهمات اليومية والربح الحقيقي\n"
        f"━━━━━━━━━━━━━━━━\n💎 أكمل مهامك اليومية واربح\n"
        f"👥 ادعُ أصدقاءك واربح من 6 مستويات\n💸 اسحب أرباحك عبر USDT TRC20\n"
        f"━━━━━━━━━━━━━━━━\n{ref_msg}"
        f"📱 أرسل *رقم هاتفك* مع رمز الدولة للتسجيل:\n\n"
        f"🇮🇶 `+9647701234567`\n🇸🇦 `+966501234567`\n🇦🇪 `+971501234567`",
        reply_markup=ReplyKeyboardRemove(),parse_mode="Markdown")

async def msg_handler(update:Update,context:ContextTypes.DEFAULT_TYPE):
    uid=update.effective_user.id; state=context.user_data.get("state")
    text=update.message.text.strip() if update.message.text else ""

    if text in [BTN_HOME,BTN_BACK]:
        context.user_data.clear(); await send_home(update.message,uid); return
    if text==BTN_ADMIN and is_admin(uid):
        await show_admin(update.message,uid); return
    if text==BTN_TASKS: await show_tasks(update.message,uid); return
    if text==BTN_WALLET: await show_wallet(update.message,uid); return
    if text==BTN_PACKAGES: await show_packages(update.message); return
    if text==BTN_REFERRAL: await show_referrals(update.message,uid,context); return
    if text==BTN_WITHDRAW: await start_withdraw(update.message,uid,context); return
    if text==BTN_HELP: await show_help(update.message); return

    # ── رقم الهاتف: يعمل دائماً لأي مستخدم غير مسجّل ──
    if text.startswith("+") and len(text)>=10 and not get_user(uid):
        full_name=update.effective_user.full_name or update.effective_user.first_name or "مستخدم"
        referred_by=context.user_data.get("referred_by")
        register_user(uid,update.effective_user.username or "",full_name,text,referred_by)
    ...
    if state==ST_RECEIPT:
        ...
        else: await update.message.reply_text("❌ أرسل صورة الوصل فقط."); return
        conn=db();c=conn.cursor()
        ...
        kb=InlineKeyboardMarkup([[InlineKeyboardButton(f"✅ قبول #{rid}",callback_data=f"ap_{rid}"),
            InlineKeyboardButton(f"❌ رفض #{rid}",callback_data=f"rj_{rid}")]])
        for adm in get_all_admins():
            try:
                if update.message.photo: await context.bot.send_photo(adm,file_id,caption=caption,reply_markup=kb,parse_mode="Markdown")
                else: await context.bot.send_document(adm,file_id,caption=caption,reply_markup=kb,parse_mode="Markdown")
            except: pass
        context.user_data.clear()
    ...
    if state==ST_SCREENSHOT:
        if not update.message.photo: await update.message.reply_text("❌ أرسل صورة فقط."); return
        file_id=update.message.photo[-1].file_id
        task_num=context.user_data.get("task_num",1); today=date.today().isoformat()
        conn=db();c=conn.cursor()
        ...
        kb=InlineKeyboardMarkup([[InlineKeyboardButton(f"✅ قبول #{sid}",callback_data=f"sap_{sid}"),
        ...
        context.user_data.clear()
        await update.message.reply_text("✅ *تم إرسال السكرين شوت!*\n\n⏳ سيتم مراجعته",
            reply_markup=main_kb(),parse_mode="Markdown"); return

    # ── محفظة المستخدم ──
    if state==ST_WALLET:
        if len(text)<30: await update.message.reply_text("❌ عنوان غير صحيح."); return
        conn=db();c=conn.cursor()
    ...
    if state==ST_WITHDRAW:
        u=get_user(uid); balance=u[5]; wallet=u[7]
        try: amount=float(text)
        except: await update.message.reply_text("❌ أدخل رقماً صحيحاً."); return
        if amount<MIN_WITHDRAW: await update.message.reply_text(f"❌ الحد الأدنى {MIN_WITHDRAW}$"); return
        if amount>balance: await update.message.reply_text(f"❌ رصيدك غير كافٍ ({balance:.2f}$)"); return
        conn=db();c=conn.cursor()
        ...
        kb=InlineKeyboardMarkup([[InlineKeyboardButton(f"✅ قبول #{wid}",callback_data=f"wap_{wid}"),
            InlineKeyboardButton(f"❌ رفض #{wid}",callback_data=f"wrj_{wid}")]])
        for adm in get_all_admins():
            try: await context.bot.send_message(adm,
                f"💸 *طلب سحب #{wid}*\n\n👤 {u[3]} | #{u[1]:04d}\n📱 {u[4]}\n💰 *{amount:.2f}$*\n👛 `{wallet}`",
                reply_markup=kb,parse_mode="Markdown")
            except: pass
        context.user_data.clear()
    ...
    if state==ST_BROADCAST and is_admin(uid):
        conn=db();c=conn.cursor();c.execute("SELECT user_id FROM users");users=c.fetchall();conn.close()
        sent=failed=0
        for (u_id,) in users:
            try: await context.bot.send_message(u_id,text); sent+=1
            except: failed+=1
        context.user_data.clear()
        await update.message.reply_text(f"✅ أُرسلت لـ {sent} | ❌ فشل مع {failed}",reply_markup=admin_kb()); return

    if state==ST_SETWALLET and is_admin(uid):
        if len(text)<20: await update.message.reply_text("❌ عنوان غير صحيح."); return
    ...
    if state==ST_ADDLINK and is_admin(uid):
        if not text.startswith("http"): await update.message.reply_text("❌ أرسل رابطاً صحيحاً."); return
        conn=db();c=conn.cursor();c.execute("INSERT INTO task_links(url)VALUES(?)",(text,))
        conn.commit();conn.close();context.user_data.clear()
        await update.message.reply_text(f"✅ تمت إضافة الرابط:\n{text}",reply_markup=admin_kb()); return

async def show_tasks(message,uid):
    u=get_user(uid)
    if not u or not u[6] or not pkg_active(u):
        await message.reply_text("❌ *ليس لديك باقة مفعّلة*\n\nاشترِ باقة لتبدأ!",
            reply_markup=back_kb(),parse_mode="Markdown"); return
    pkg=PACKAGES[u[6]]; today_row=get_today(uid)
...
async def show_wallet(message,uid):
    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(
            "✏️ تغيير المحفظة" if wallet else "➕ أضف محفظة",callback_data="add_wallet")]]),
        parse_mode="Markdown")
...
async def show_referrals(message,uid,context):
    ...
    u=get_user(uid); ref_code=u[8]
    link=f"https://t.me/{bot_username}?start={ref_code}"
    conn=db();c=conn.cursor()
    c.execute("SELECT COUNT(*) FROM users WHERE referred_by=?",(uid,));direct=c.fetchone()[0]
    c.execute("SELECT SUM(amount) FROM referral_earnings WHERE user_id=?",(uid,));total_e=c.fetchone()[0] or 0
    ...
    for lvl in range(1,7):
        cnt,earned=lvls.get(lvl,(0,0)); text+=f"L{lvl}: {cnt} شخص — {earned:.2f}$\n"
    await message.reply_text(text,reply_markup=back_kb(),parse_mode="Markdown")
...
async def show_admin(message,uid):
    total,pp,wp,sp,tb=get_stats(); tasks_open=is_tasks_open()
    toggle="🔴 قفل المهام" if tasks_open else "🟢 فتح المهام"
    await message.reply_text(
        f"👑 *لوحة الأدمن*\n\n👥 المستخدمون: *{total}*\n⏳ طلبات شراء: *{pp}*\n"
        f"💸 طلبات سحب: *{wp}*\n📸 سكرين شوت: *{sp}*\n💰 الأرصدة: *{tb:.2f}$*\n\n{tasks_txt()}",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("👥 المستخدمون",callback_data="au"),InlineKeyboardButton("📊 الإحصائيات",callback_data="as_")],
            [InlineKeyboardButton("⏳ طلبات الشراء",callback_data="ap_list"),InlineKeyboardButton("💸 طلبات السحب",callback_data="aw_list")],
            [InlineKeyboardButton("📸 سكرين شوت",callback_data="ass"),InlineKeyboardButton("🎬 الفيديوهات",callback_data="al")],
            [InlineKeyboardButton("👛 تغيير المحفظة",callback_data="awt"),InlineKeyboardButton(toggle,callback_data="at")],
            [InlineKeyboardButton("📢 رسالة جماعية",callback_data="ab")]
        ]),parse_mode="Markdown")

def admin_inline_kb(tasks_open):
    toggle="🔴 قفل المهام" if tasks_open else "🟢 فتح المهام"
...
async def btn_handler(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("do_task_"):
        task_num=int(data[8:]); context.user_data["state"]=ST_SCREENSHOT; context.user_data["task_num"]=task_num
        await q.edit_message_text(
            f"📸 *تأكيد المهمة {task_num}*\n\nأرسل سكرين شوت يُثبت:\n✅ مشاهدة الفيديو\n✅ الاشتراك بالقناة\n✅ الإعجاب بالفيديو\n\n"
            f"⚠️ الصورة لازم تكون من داخل يوتيوب",parse_mode="Markdown")
        ...
        pkg_name=data[4:]; pkg=PACKAGES[pkg_name]; comms=REFERRAL_COMMISSIONS[pkg_name]; wallet=get_setting("wallet")
        await q.edit_message_text(
            f"📦 *باقة {pkg_name}*\n\n💰 السعر: *{pkg['price']}$*\n📋 *{pkg['tasks']} مهام/يوم*\n"
            f"💵 ربح المهمة: *{pkg['per_task']:.2f}$*\n📈 اليومي: *{pkg['daily']:.2f}$*\n📅 المدة: *{PACKAGE_DAYS} يوم*\n\n"
        ...
        kb=[]
        for name,pkg in PACKAGES.items():
            kb.append([InlineKeyboardButton(f"{'⭐ ' if name=='B1' else ''}{name} — {pkg['price']}$ ({pkg['daily']:.2f}$/يوم)",callback_data=f"buy_{name}")])
        await q.edit_message_text(f"📦 *اختر باقتك*\n\n✅ صالحة لمدة *{PACKAGE_DAYS} يوم*",
            reply_markup=InlineKeyboardMarkup(kb),parse_mode="Markdown")

    elif data.startswith("sr_"):
        pkg_name=data[3:]; context.user_data["buying_package"]=pkg_name; context.user_data["state"]=ST_RECEIPT
        await q.edit_message_text(f"📤 *وصل الدفع — باقة {pkg_name}*\n\nأرسل صورة الوصل الآن 👇\n\n⏳ التفعيل خلال 24 ساعة",parse_mode="Markdown")

    elif data.startswith("sap_") and is_admin(uid):
        sid=int(data[4:]); conn=db(); c=conn.cursor()
        ...
        target_user=get_user(target_uid); per_task=0
        if target_user and target_user[6]:
            per_task=PACKAGES[target_user[6]]["per_task"]
            c.execute("SELECT tasks_done FROM daily_tasks WHERE user_id=? AND task_date=?",(target_uid,task_date))
            tr=c.fetchone()
            if tr: c.execute("UPDATE daily_tasks SET tasks_done=tasks_done+1,earned=earned+? WHERE user_id=? AND task_date=?",(per_task,target_uid,task_date))
            else: c.execute("INSERT INTO daily_tasks(user_id,task_date,tasks_done,earned)VALUES(?,?,?,?)",(target_uid,task_date,1,per_task))
            c.execute("UPDATE users SET balance=balance+? WHERE user_id=?",(per_task,target_uid))
        conn.commit();conn.close()
        try: await context.bot.send_message(target_uid,f"✅ *تمت الموافقة على مهمتك!*\n\n💰 ربحت: *{per_task:.2f}$*",reply_markup=main_kb(),parse_mode="Markdown")
        except: pass
        ...
        sid=int(data[4:]); conn=db(); c=conn.cursor()
        c.execute("SELECT user_id FROM task_screenshots WHERE id=?",(sid,)); row=c.fetchone()
        ...
        rid=int(data[3:]); conn=db(); c=conn.cursor()
        c.execute("SELECT user_id,package FROM packages_history WHERE id=? AND status='pending'",(rid,)); row=c.fetchone()
        if not row: await q.answer("❌ مُعالَج مسبقاً"); conn.close(); return
        target_uid,pkg_name=row
        c.execute("UPDATE packages_history SET status='approved' WHERE id=?",(rid,))
        conn.commit();conn.close()
        activate_pkg(target_uid,pkg_name); distribute_commissions(target_uid,pkg_name)
        expires=(datetime.now()+timedelta(days=PACKAGE_DAYS)).strftime("%Y-%m-%d")
        try: await context.bot.send_message(target_uid,f"🎉 *تم تفعيل باقتك {pkg_name}!*\n\n📅 صالحة حتى: *{expires}*\n\nابدأ بإكمال مهامك 💰",reply_markup=main_kb(),parse_mode="Markdown")
        except: pass
        ...
        rid=int(data[3:]); conn=db(); c=conn.cursor()
        c.execute("SELECT user_id FROM packages_history WHERE id=?",(rid,)); row=c.fetchone()
        ...
        wid=int(data[4:]); conn=db(); c=conn.cursor()
        c.execute("SELECT user_id,amount,wallet FROM withdrawals WHERE id=? AND status='pending'",(wid,)); row=c.fetchone()
        if not row: await q.answer("❌ مُعالَج مسبقاً"); conn.close(); return
        target_uid,amount,wallet=row
        c.execute("UPDATE withdrawals SET status='approved' WHERE id=?",(wid,))
        conn.commit();conn.close()
        try: await context.bot.send_message(target_uid,f"✅ *تم إرسال سحبك!*\n\nالمبلغ: *{amount:.2f}$*\nالمحفظة: `{wallet}`",parse_mode="Markdown")
        except: pass
        await q.edit_message_text(f"✅ تم قبول السحب #{wid} — {amount}$")

    elif data.startswith("wrj_") and is_admin(uid):
        wid=int(data[4:]); conn=db(); c=conn.cursor()
        c.execute("SELECT user_id,amount FROM withdrawals WHERE id=? AND status='pending'",(wid,)); row=c.fetchone()
        if not row: await q.answer("❌ مُعالَج مسبقاً"); conn.close(); return
        ...
        cur=get_setting("tasks_open"); new="0" if cur=="1" else "1"; set_setting("tasks_open",new)
        total,pp,wp,sp,tb=get_stats()
        status="🟢 مفتوحة" if new=="1" else "🔴 مغلقة"
        await q.edit_message_text(f"✅ المهام الآن: *{status}*\n\n{tasks_txt()}",
            reply_markup=admin_inline_kb(new=="1"),parse_mode="Markdown")
        ...
        await q.edit_message_text(text,reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع",callback_data="aback")]]),parse_mode="Markdown")
        ...
        conn=db();c=conn.cursor()
        c.execute("SELECT COUNT(*) FROM task_screenshots WHERE status='pending'");sp=c.fetchone()[0];conn.close()
        await q.edit_message_text(f"📸 *سكرين شوت المهام*\n\nمعلق: *{sp}*\n\nالصور تُرسل لك مباشرة مع أزرار القبول والرفض 👆",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع",callback_data="aback")]]),parse_mode="Markdown")

    elif data=="al" and is_admin(uid):
        conn=db();c=conn.cursor();c.execute("SELECT id,url FROM task_links WHERE active=1");links=c.fetchall();conn.close()
        text="🎬 *روابط المهام:*\n\n"; kb=[]
        for lid,url in links:
            text+=f"• {url[:60]}\n"
            kb.append([InlineKeyboardButton(f"🗑 حذف #{lid}",callback_data=f"dl_{lid}")])
        kb+=[[InlineKeyboardButton("➕ إضافة رابط",callback_data="addl")],[InlineKeyboardButton("🔙 رجوع",callback_data="aback")]]
        await q.edit_message_text(text,reply_markup=InlineKeyboardMarkup(kb),parse_mode="Markdown")

    elif data=="addl" and is_admin(uid):
        context.user_data["state"]=ST_ADDLINK; await q.edit_message_text("🎬 أرسل رابط يوتيوب الجديد:")

    elif data.startswith("dl_") and is_admin(uid):
        lid=int(data[3:]); conn=db();c=conn.cursor()
        c.execute("UPDATE task_links SET active=0 WHERE id=?",(lid,));conn.commit();conn.close()
        await q.answer("✅ تم حذف الرابط")
        conn=db();c=conn.cursor();c.execute("SELECT id,url FROM task_links WHERE active=1");links=c.fetchall();conn.close()
        text="🎬 *روابط المهام:*\n\n"; kb=[]
        for l_id,url in links:
            text+=f"• {url[:60]}\n"; kb.append([InlineKeyboardButton(f"🗑 حذف #{l_id}",callback_data=f"dl_{l_id}")])
        kb+=[[InlineKeyboardButton("➕ إضافة",callback_data="addl")],[InlineKeyboardButton("🔙 رجوع",callback_data="aback")]]
        await q.edit_message_text(text,reply_markup=InlineKeyboardMarkup(kb),parse_mode="Markdown")

    elif data=="awt" and is_admin(uid):
        context.user_data["state"]=ST_SETWALLET; wallet=get_setting("wallet")
        await q.edit_message_text(f"👛 *تغيير محفظة الاستلام*\n\nالحالية:\n`{wallet}`\n\nأرسل العنوان الجديد:",parse_mode="Markdown")

    elif data=="ab" and is_admin(uid):
        context.user_data["state"]=ST_BROADCAST; await q.edit_message_text("📢 أرسل الرسالة الجماعية الآن:")

    elif data=="aback" and is_admin(uid):
        total,pp,wp,sp,tb=get_stats()
        await q.edit_message_text(
            f"👑 *لوحة الأدمن*\n\n👥 {total} | ⏳{pp} | 💸{wp} | 📸{sp}\n\n{tasks_txt()}",
            reply_markup=admin_inline_kb(is_tasks_open()),parse_mode="Markdown")
...
async def makeadmin_cmd(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args: await update.message.reply_text("❌ الصيغة: /makeadmin user_id"); return
    try:
        target_id=int(context.args[0]); conn=db();c=conn.cursor()
        c.execute("UPDATE users SET is_admin=1 WHERE user_id=?",(target_id,));conn.commit();conn.close()
        target=get_user(target_id); name=target[3] if target else str(target_id)
        await update.message.reply_text(f"✅ تم تعيين *{name}* أدمناً مشاركاً",parse_mode="Markdown")
        try: await context.bot.send_message(target_id,"🎉 *تم تعيينك أدمناً مشاركاً!*\n\nاستخدم /admin للوحة التحكم",reply_markup=admin_kb(),parse_mode="Markdown")
        except: pass
    except Exception as e: await update.message.reply_text(f"❌ خطأ: {e}")

async def removeadmin_cmd(update:Update,context:ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id!=MAIN_ADMIN: return
    if not context.args: await update.message.reply_text("❌ الصيغة: /removeadmin user_id"); return
    try:
        target_id=int(context.args[0]); conn=db();c=conn.cursor()
        c.execute("UPDATE users SET is_admin=0 WHERE user_id=?",(target_id,));conn.commit();conn.close()
...
async def activate_cmd(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args)<2: await update.message.reply_text("❌ الصيغة: /activate user_id باقة\nمثال: /activate 123456 A1"); return
    try:
        target_id=int(context.args[0]); pkg_name=context.args[1].upper()
        if pkg_name not in PACKAGES: await update.message.reply_text(f"❌ الباقات: {', '.join(PACKAGES.keys())}"); return
        activate_pkg(target_id,pkg_name); distribute_commissions(target_id,pkg_name)
        expires=(datetime.now()+timedelta(days=PACKAGE_DAYS)).strftime("%Y-%m-%d")
        target=get_user(target_id); name=target[3] if target else str(target_id)
        await update.message.reply_text(f"✅ تم تفعيل باقة *{pkg_name}* للمستخدم *{name}*\n📅 تنتهي: {expires}",parse_mode="Markdown")
        try: await context.bot.send_message(target_id,f"🎉 *تم تفعيل باقتك {pkg_name}!*\n\n📅 صالحة حتى: *{expires}*\n\nابدأ بإكمال مهامك 💰",reply_markup=main_kb(),parse_mode="Markdown")
        except: pass
    except Exception as e: await update.message.reply_text(f"❌ خطأ: {e}")
